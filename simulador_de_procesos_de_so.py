...
class Ventana(QMainWindow):
 #Método constructor de la clase
	def __init__(self):
		...
		self.algoritmos_P_Alta.currentTextChanged.connect(self.quantum_alta)
		self.algoritmos_P_Media.currentTextChanged.connect(self.quantum_media)
			...
			try:
				...
				with open(filename[0], 'r') as f:
					...
					while file_text !='':			
						...
						file_text=file_text.split('|')
						self.tab_procesos.setRowCount(self.tab_procesos.rowCount()+1)
						p=QTableWidgetItem(str(int(file_text[0]) + int(self.tab_procesos.rowCount()- cont) ))
						p.setTextAlignment(Qt.AlignCenter)
						tam=QTableWidgetItem(str(file_text[1]))
						tam.setTextAlignment(Qt.AlignCenter)
						arribo=QTableWidgetItem(str(file_text[2]))
	...
	def quantum_alta(self): #para el combo box de la priridad media en cm
	...
	def quantum_media(self): #para el combo box de la priridad baja en cm
